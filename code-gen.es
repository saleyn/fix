...
generate_fields(Fields, FldMap, #state{var_sfx=SFX} = State) ->
  ...
  ok  = write_file(erlang, src, State, FN2++".erl", [], [
    ...
    "\n"
    "-spec encode_msg(binary() | proplists:proplist(), binary()) -> binary().\n"
  ...
  [
    ...
    "%% @see fix_nif:decode_field_value/2.\n"
    "decode_field_value(ID, Value) ->\n"
    "  fix_nif:decode_field_value(?FIX_VARIANT, ID, Value).\n\n"
    "%% @doc Decode field value with format\n"
    "%% @see fix_nif:decode_field_value/3.\n"
    "decode_field_value(ID, Value, Format) ->\n"
    "  fix_nif:decode_field_value(?FIX_VARIANT, ID, Value, Format).\n\n"
    "%% @see decode/3.\n"
    ...
    "-spec decode(nif|native, binary(), fix_nif:decode_options()) ->\n"
    "        {ok, Rest::binary(), fix_util:success_decode()}\n"
    ...
    "-spec split(atom()|binary(), binary()|fix_nif:decode_options()) ->\n"
    ...
    "-spec split(atom(), binary(), fix_nif:decode_options()) -> fix_nif:split_message().\n"
    ...
    "\n"
    "-spec encode_msg_nif(proplists:proplist()|[{atom(),any(),integer(),any()}]) -> iolist().\n"
